...
import { DiscoveredExtension, ExtensionConfigDescriptor, ExtensionName, InternalExtensionInfo } from './types';
...
export class ExtensionsService implements CoreService<ExtensionsServiceSetup, ExtensionsServiceStart> {
  ...
  private async handleDiscoveredExtensions(extension$: Observable<ExtensionWrapper>) {
    ...
    await extension$
      .pipe(
        mergeMap(async (extension) => {
          ...
          const isEnabled = await this.coreContext.configService.isEnabledAtPath(extension.configPath);
    ...
    for (const [extensionName, { extension, isEnabled }] of extensionEnableStatuses) {
      ...
      const extensionEnablement = this.shouldEnableExtension(extensionName, extensionEnableStatuses);
  ...
  private shouldEnableExtension(
    ...
    extensionEnableStatuses: Map<ExtensionName, { extension: ExtensionWrapper; isEnabled: boolean }>,
    ...
    const missingDependencies = extensionInfo.extension.requiredExtensions
      ...
      .filter(
        (dependencyName) =>
          !this.shouldEnableExtension(dependencyName, extensionEnableStatuses, [...parents, extensionName])
            .enabled
      );
