...
export class ExtensionsSystem {
      ...
      [...this.extensions].map(([name, extension]) => [
        ...
        [
          ...new Set([
            ...
            ...extension.optionalExtensions.filter((optExtension) => this.extensions.has(optExtension)),
          ]),
  ...
  public async setupExtensions(deps: ExtensionsServiceSetupDeps) {
    ...
    const sortedExtensions = new Map(
      [...this.getTopologicallySortedExtensionNames()]
        .map((extensionName) => [extensionName, this.extensions.get(extensionName)!] as [string, ExtensionWrapper])
    ...
    for (const [extensionName, extension] of sortedExtensions) {
      ...
      const extensionDeps = new Set([...extension.requiredExtensions, ...extension.optionalExtensions]);
      const extensionDepContracts = Array.from(extensionDeps).reduce((depContracts, dependencyName) => {
  ...
  public async startExtensions(deps: ExtensionsServiceStartDeps) {
    ...
    this.log.info(`Starting [${this.satupExtensions.length}] extensions: [${[...this.satupExtensions]}]`);
