...
> {
   ...
   */
  public async setup(setupContext: CoreSetup<TExtensionsStart>, extensions: TExtensionsSetup) {
  ...
  private createExtensionInstance() {
    ...
    if (!('extension' in extensionDefinition)) {
      throw new Error(`Extension "${this.name}" does not export "extension" definition (${this.path}).`);
    }
    ...
    if (!initializer || typeof initializer !== 'function') {
      throw new Error(`Definition of extension "${this.name}" should be a function (${this.path}).`);
    }
